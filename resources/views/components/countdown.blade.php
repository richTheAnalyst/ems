            ...
            @foreach(['days', 'hours', 'minutes', 'seconds'] as $label)
                <div class="text-center">
                    <div class="bg-[#d9d9d9] rounded-[30px] p-8 mb-4 transition-all duration-300 hover:bg-gray-400 hover:shadow-lg">
                        <span class="text-4xl font-bold text-black transition-colors duration-300 hover:text-green" id="{{ $label }}">0</span>
                    </div>
                    <span class="text-xl font-bold text-black transition-transform duration-300 hover:scale-110">{{ strtoupper($label) }}</span>
                </div>
            @endforeach
        </div>
    </div>  
